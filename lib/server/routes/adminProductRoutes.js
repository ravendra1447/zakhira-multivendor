    ...
    if (stockMode === 'simple') {
      ...
      if (!isNaN(variantId)) {
        ...
        if (existingVariants.length === 0) {
          console.log(`No variants found for product ${productId}! Creating mock variants first...`);
          
          // Create mock variants for this product
          const mockColors = [
          ...
          const mockSizes = ['6', '7', '8', '9', '10'];
          
          for (const colorData of mockColors) {
            for (const size of mockSizes) {
              await pool.execute(`
                INSERT INTO product_variants (product_id, color_name, color_code, size, price, stock, status) 
                VALUES (?, ?, ?, ?, ?, ?, ?)
              `, [productId, colorData.color, colorData.color_code, size, 750, 10, 1]);
            }
          }
          
          console.log(`Created ${mockColors.length * mockSizes.length} mock variants for product ${productId}`);
...
router.get('/admin/products/:productId/variants', async (req, res) => {
  try {
    ...
    // Force color_size mode for testing if no variants exist
    if ((stockMode === 'simple' || stockMode === 'always_available') && productId) {
      console.log(`Product ${productId} is in ${stockMode} mode, forcing to color_size mode to show variants...`);
      // You can uncomment this line to force color_size mode for testing
      stockMode = 'color_size';
      console.log(`Forced stockMode to color_size for product ${productId}`);
    }
    ...
    if (stockMode === 'simple') {
      ...
      // If no variants found but stock_mode is color_size, create real variants in database
      if (colorGroups.length === 0 && stockMode === 'color_size') {
        console.log(`No variants found for product ${productId}, creating real variants in database`);
        
        // Create mock colors and sizes
        const mockColors = [
          { color: 'Black', color_code: '#000000' },
          { color: 'White', color_code: '#FFFFFF' },
          { color: 'Blue', color_code: '#0000FF' },
        ...
        // First, add colors to product_colors table
        for (const colorData of mockColors) {
          await pool.execute(`
            INSERT IGNORE INTO product_colors (product_id, color_name, color_code, price, stock) 
            VALUES (?, ?, ?, ?, ?)
          `, [productId, colorData.color, colorData.color_code, productStock || 750, 10]);
        }
        
        // Then, add variants to product_variants table
        for (const colorData of mockColors) {
          const colorVariants = [];
          for (const size of mockSizes) {
            const [insertResult] = await pool.execute(`
              INSERT INTO product_variants (product_id, color_name, color_code, size, price, stock, status) 
              VALUES (?, ?, ?, ?, ?, ?, ?)
            `, [productId, colorData.color, colorData.color_code, size, productStock || 750, 10, 1]);
            
            const insertedId = insertResult.insertId;
            console.log(`Inserted variant with database ID: ${insertedId}`);
            
            colorVariants.push({
              id: insertedId, // Use the actual database ID
              color: colorData.color,
              size: size,
              stock: 10,
              price: productStock || 750,
              is_active: true,
          ...
          colorGroups.push({
            ...
            image_url: null,
            color_status: 1,
            total_sizes: mockSizes.length,
            active_sizes: mockSizes.length,
            variants: colorVariants
          });
        }
        
        console.log(`Created ${mockColors.length * mockSizes.length} real variants in database for product ${productId}`);
        
        // After creating variants, refetch them to get the real database data
        const [newVariants] = await pool.execute(`
          SELECT 
            pv.id,
            pv.product_id,
