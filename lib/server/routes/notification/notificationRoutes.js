...
// Send marketplace chat notification
router.post('/marketplace-chat', async (req, res) => {
  try {
    const { sellerId, chatRoomId, senderId, message, productInfo } = req.body;
    
    if (!sellerId || !chatRoomId || !senderId || !message) {
      return res.status(400).json({
        success: false,
        message: 'Seller ID, Chat Room ID, Sender ID and Message are required'
      });
    }
    ...
    if (seller.fcm_token) {
      try {
        const title = productInfo ? `New inquiry about ${productInfo.product_name}` : 'New Message';
        const body = message;
        ...
        console.log(`📱 Marketplace chat notification sent to seller ${sellerId}`);
        
      } catch (fcmError) {
        console.log('⚠️ FCM notification failed:', fcmError.message);
      }
    }

    res.json({
      success: true,
      message: 'Marketplace chat notification sent successfully',
      notifications: {
        fcm: seller.fcm_token ? 'sent' : 'no_fcm_token'
      }
    });

  } catch (error) {
    console.error('❌ Error sending marketplace chat notification:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to send marketplace chat notification'
    });
  }
});
router.post('/order-confirmation', async (req, res) => {
  try {
    ...
    const [orders] = await pool.execute(`
      SELECT id, total_amount, order_status, order_date, 
    ...
    // 3. Send email notification (only if user has email)
    console.log(`🔍 Email validation check for user ${user.name}:`, {
      email: user.email,
      emailType: typeof user.email,
      isNull: user.email === null,
      isUndefined: user.email === undefined,
      isEmptyString: user.email === '',
      trimResult: user.email ? user.email.trim() : 'N/A'
    });
    ...
    if (user.email && user.email.trim() !== '') {
      ...
      console.log(`📧 Email skipped for user ${user.name} - no email address provided`);
    }
...
router.post('/new-order-seller', async (req, res) => {
  try {
    ...
    // Get order items with product details to find sellers
    const [orderItems] = await pool.execute(`
      SELECT oi.*, p.user_id as seller_id, p.name as product_name
      FROM order_items oi
      INNER JOIN products p ON oi.product_id = p.id
      WHERE oi.order_id = ?
    `, [orderId]);
    ...
    const uniqueSellers = [...new Set(orderItems.map(item => item.seller_id))];
    
    // Filter out the buyer from sellers list (in case buyer bought their own product)
    const sellers = uniqueSellers.filter(sellerId => sellerId !== order.user_id);

    if (sellers.length === 0) {
      console.log('No valid sellers found for order:', orderId);
      return res.json({
        success: true,
        message: 'No sellers to notify (buyer may have purchased their own products)',
        notificationsSent: 0
      });
    }
    ...
    // Send notifications to each unique seller
    for (const sellerId of sellers) {
      // Get seller details
      const [sellerDetails] = await pool.execute(`
        SELECT user_id, name, email, fcm_token
        FROM users 
        WHERE user_id = ?
      `, [sellerId]);
      ...
      console.log(`🔍 Processing seller: ${seller.name} (ID: ${seller.user_id})`);
      console.log(`🔍 Seller FCM token: ${seller.fcm_token ? 'exists' : 'missing'}`);
      console.log(`🔍 Seller email: ${seller.email || 'missing'}`);
      
      // Get products sold by this seller in this order
      const sellerProducts = orderItems.filter(item => item.seller_id === sellerId);
      console.log(`🔍 Seller products in order: ${sellerProducts.length} items`);
      
      // 1. Send FCM notification to seller
      if (seller.fcm_token && seller.fcm_token.trim() !== '') {
        console.log(`📱 Sending FCM to seller ${seller.name} with token: ${seller.fcm_token.substring(0, 20)}...`);
        try {
          await sendFCMNotification(
            seller.fcm_token,
            'New Order Received! 📦',
            `Order #${orderId} from ${order.customer_name}. Amount: ₹${order.total_amount}`,
            {
              ...
              amount: order.total_amount.toString(),
              productCount: sellerProducts.length.toString(),
              timestamp: new Date().toISOString()
            }
          );
          console.log(`✅ FCM notification sent successfully to seller ${seller.name}`);
        } catch (fcmError) {
          console.log(`❌ FCM notification failed for seller ${seller.name}:`, fcmError.message);
        }
      } else {
        console.log(`❌ Seller ${seller.name} (ID: ${seller.user_id}) has NO FCM token! Cannot send notification.`);
        console.log(`🔍 Available seller data:`, {
      ...
      // 2. Skip email for sellers - only send FCM notification
      console.log(`📧 Email notification skipped for seller ${seller.name} - only FCM notification sent`);

      // 3. Store seller notification in database
      await pool.execute(`
        INSERT INTO notifications (user_id, type, title, message, data, created_at)
        VALUES (?, 'new_order_seller', ?, ?, ?, NOW())
      `, [
        seller.user_id,
        'New Order Received! 📦',
        `Order #${orderId} from ${order.customer_name}. Amount: ₹${order.total_amount}`,
    ...
    res.json({
      success: true,
      message: `Seller notifications sent to ${notificationsSent} sellers`,
...
async function sendFCMNotification(token, title, body, data = {}) {
  try {
    ...
    if (!admin.apps.length) {
      throw new Error('Firebase Admin SDK not initialized. Please check server.js configuration.');
    }
    ...
    if (error.code === 'messaging/registration-token-not-registered') {
      ...
      await pool.execute('UPDATE users SET fcm_token = NULL WHERE fcm_token = ?', [message.token]);
      console.log('🗑️ Invalid FCM token removed from database for user');
      
      // Don't throw error, just log it
      return { success: false, message: 'Invalid FCM token - removed from database' };
    }
...
async function sendEmail(to, subject, body) {
  try {
    ...
    if (!to || to === null || to === undefined || to.trim() === '') {
      console.log(`❌ Email sending blocked - invalid recipient: ${to}`);
...
// Generate New Order Email for Seller
function generateNewOrderSellerEmail(seller, order, sellerProducts) {
  const itemsHtml = sellerProducts.map(item => `
    <tr>
      <td style="padding: 10px; border-bottom: 1px solid #eee;">${item.product_name}</td>
      <td style="padding: 10px; border-bottom: 1px solid #eee;">${item.quantity}</td>
      <td style="padding: 10px; border-bottom: 1px solid #eee;">₹${item.price}</td>
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .order-info { padding: 20px 0; }
        .table { width: 100%; border-collapse: collapse; margin: 20px 0; }
        .alert { background-color: #4CAF50; color: white; padding: 15px; border-radius: 5px; margin-bottom: 20px; }
        .footer { text-align: center; padding: 20px 0; color: #666; }
        .seller-total { text-align: right; font-size: 18px; font-weight: bold; color: #4CAF50; margin-top: 10px; }
      </style>
    </head>
    <body>
      <div class="container">
        ...
        <div class="alert">
          <strong>Good News!</strong> You have received a new order for your products.
        </div>
