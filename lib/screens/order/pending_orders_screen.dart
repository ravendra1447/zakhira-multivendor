...
class PendingOrdersScreen extends StatefulWidget {
  ...
  const PendingOrdersScreen({super.key, this.onOrderStatusChanged});
  ...
  Future<void> _fetchPendingOrders() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${Config.baseNodeApiUrl}/orders?status=Pending'),
        headers: {
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onTap: () {
            ...
            ).then((_) => _fetchPendingOrders()); // Refresh when coming back
          },
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success']) {
          ...
          if (newOrderCount != _lastOrderCount || newOrderHash != _lastOrderHash) {
            print('🔄 Changes detected! Refreshing pending orders...');
