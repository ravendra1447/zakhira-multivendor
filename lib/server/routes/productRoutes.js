...
router.post("/save", async (req, res) => {
  ...
  try {
    ...
    const {
      ...
      company_id, // Company ID for branch management
      branch_id, // Branch ID for branch management
    } = req.body;

    // Check if user has company assigned (required only for PUBLISHED products, not drafts)
    const [userInfo] = await connection.execute(
      'SELECT company_id, branch_id, phone FROM users WHERE user_id = ?',
      [user_id]
    );
    
    // Only require company registration for published products
    if (status === 'publish' && (userInfo.length === 0 || !userInfo[0].company_id || !userInfo[0].branch_id)) {
      return res.status(403).json({ 
        success: false, 
        message: 'Please register your company first to publish products',
        needs_company_registration: true,
        user_phone: userInfo.length > 0 ? userInfo[0].phone : null
      });
    }
    
    // Get user's company and branch info
    // For draft products, company/branch can be null
    let userCompanyId = null;
    let userBranchId = null;
    
    if (userInfo.length > 0 && userInfo[0].company_id && userInfo[0].branch_id) {
      userCompanyId = company_id || userInfo[0].company_id;
      userBranchId = branch_id || userInfo[0].branch_id;
    } else if (status === 'publish') {
      // This should not happen due to the check above, but just in case
      return res.status(403).json({ 
        success: false, 
        message: 'Company registration required for published products',
        needs_company_registration: true
      });
    }
    ...
    if (product_id) {
      ...
      const updateParams = [
        ...
        userCompanyId || null,
        userBranchId || null,
        user_id, // published_by is the same as user_id for now
      ];
      ...
      let insertQuery = `INSERT INTO products (
          ...
          company_id, branch_id, published_by`;
...
router.post("/get", async (req, res) => {
  try {
    ...
    try {
      ...
      // Build attributes object and selectedAttributeValues
      const attributesObj = {};
      ...
      for (const attr of attributes) {
        ...
        if (attrName) {
          // For now, we'll create a simple structure. In a full implementation,
          // you might want to get all possible values from master table
          attributesObj[attrName] = [attrName]; // Simple array with current value
          selectedAttributeValues[attrName] = attrName; // Current selected value
        }
      }

      // Parse existing JSON fields from product
      ...
      // Merge database attributes with JSON attributes (prioritize database)
      const finalAttributes = { ...parsedAttributes, ...attributesObj };
      const finalSelectedAttributeValues = { ...parsedSelectedAttributeValues, ...selectedAttributeValues };
