...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `?? [PARALLEL PROCESSING] Processing image ${parseInt(image_index) + 1}/${
          groupSession.total_images
        } for group ${group_upload_id}`
      );
      console.log(
        `   - Currently processing: ${groupSession.uploaded_images + 1} images in parallel`
      );

      // ? STEP 1: GENERATE THUMBNAIL IMMEDIATELY (PARALLEL PROCESSING)
      ...
      // Generate thumbnail only (this runs in parallel for multiple requests)
      const thumbnailStartTime = Date.now();
      if ([".jpg", ".jpeg", ".png", ".webp"].includes(ext)) {
        console.log(
          `?? [PARALLEL] Generating optimized thumbnail for image ${image_index}...`
        );
        thumbnailBase64 = await generateThumbnail(fileBuffer);
      }
      const thumbnailTime = Date.now() - thumbnailStartTime;
      console.log(
        `   - Thumbnail generated in ${thumbnailTime}ms (parallel processing)`
      );
          ...
          if (!groupSession.thumbnails_sent_list.includes(imageIdx)) {
            ...
            if (
              ...
              if (io && groupSession.actual_images_ready.length > 0) {
                ...
                // ? Send all ready actual images IN PARALLEL (WhatsApp style - fast)
                await Promise.all(
                  groupSession.actual_images_ready.map(async (actualImageData) => {
                  ...
                  if (
                    ...
                    console.log(
                      `[ACTUAL IMAGE SENT IN PARALLEL] Image ${
                        actualImageData.image_index + 1
                ...
                console.log(
                  `[COMPLETE] Sent ${groupSession.actual_images_sent} actual images in PARALLEL after all thumbnails`
                );
              }
            }
          }
      ...
      // ? STEP 4: BACKGROUND PROCESSING FOR ACTUAL IMAGE (NON-BLOCKING - PARALLEL)
      // ? This runs in parallel for all images simultaneously
      setImmediate(async () => {
        ...
        try {
          console.log(
            `?? [PARALLEL BACKGROUND] Background processing started for image ${image_index}...`
          );
          console.log(
            `   - Processing in parallel with other images (non-blocking)`
          );
...
const UPLOAD_BASE_URL = process.env.UPLOAD_BASE_URL || "https://bangkokmart.in/uploads";
const fullMediaUrl = `${UPLOAD_BASE_URL}/file/${mediaUrl}`;
