...
const { setupMarketplaceSocket } = require('./marketplace_socket');
...
// Redirect old payment links to new path
app.get('/api/whatsapp/payment-qr/:orderId', (req, res) => {
  const { orderId } = req.params;
  res.redirect(`/api/whatsapp-payment/payment-qr/${orderId}`);
});
...
app.use("/api/whatsapp", whatsappPaymentRoutes);
...
app.post("/api/debug-post", (req, res) => {
  console.log('🔍 DEBUG POST REQUEST:', req.body);
  res.json({
    success: true,
...
// Debug profile route
app.post("/api/profile/debug", async (req, res) => {
  try {
    console.log('🔍 DEBUG PROFILE REQUEST:', req.body);
...
// Product routes for HTML pages with Open Graph tags
