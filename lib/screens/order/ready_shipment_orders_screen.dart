...
class ReadyShipmentOrdersScreen extends StatefulWidget {
  final Function(String, String)? onOrderStatusChanged;

  const ReadyShipmentOrdersScreen({super.key, this.onOrderStatusChanged});

  @override
  State<ReadyShipmentOrdersScreen> createState() => _ReadyShipmentOrdersScreenState();
}

class _ReadyShipmentOrdersScreenState extends State<ReadyShipmentOrdersScreen> {
  List<dynamic> orders = [];
  bool isLoading = true;
  String? errorMessage;
  ...
  Future<void> _fetchReadyShipmentOrders() async {
    try {
      setState(() {
        isLoading = true;
        errorMessage = null;
      ...
      final response = await http.get(
        Uri.parse('${Config.baseNodeApiUrl}/orders?status=Ready for Shipment'),
        headers: {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: _fetchReadyShipmentOrders,
  ...
  Widget _buildOrdersList() {
    return RefreshIndicator(
      onRefresh: _fetchReadyShipmentOrders,
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onTap: () {
            ...
            ).then((_) => _fetchReadyShipmentOrders()); // Refresh when coming back
          },
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success']) {
          ...
          if (newOrderCount != _lastOrderCount || newOrderHash != _lastOrderHash) {
            print('🔄 Changes detected! Refreshing ready shipment orders...');
  ...
  Future<void> _refreshDataSilently() async {
    ...
    try {
      await _fetchReadyShipmentOrders();
    } catch (e) {
