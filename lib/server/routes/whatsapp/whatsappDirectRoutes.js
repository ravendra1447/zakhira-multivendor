...
router.get('/payment-qr/:orderId', async (req, res) => {
  try {
    ...
    // Fetch order amount from database
    let amount = 0;
    try {
      ...
      if (orders.length > 0) {
        amount = parseFloat(orders[0].total_amount) || 0;
      }
    } catch (e) {
    ...
    console.log(`[PaymentQR] Order: ${orderId}, Amount Raw: ${amount}, Type: ${typeof amount}`);
    ...
    <body>
        ...
        <script>
            ...
            function checkStatus() {
                // Poll for status
                fetch('/api/whatsapp/check-payment-status/' + orderId)
                    .then(res => res.json())
                    .then(data => {
