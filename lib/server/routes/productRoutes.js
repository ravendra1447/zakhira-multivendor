...
router.post("/create-catalog", async (req, res) => {
  ...
  try {
    ...
    // Insert new catalog
    // Get first product image for catalog
    let firstProductImage = null;
    if (product_ids.length > 0) {
      const [firstProduct] = await connection.execute(
        "SELECT images FROM products WHERE id = ? AND user_id = ? AND images IS NOT NULL AND images != ''",
        [product_ids[0], user_id]
      );
      
      if (firstProduct.length > 0) {
        const images = firstProduct[0].images;
        // Handle different image formats (JSON array, comma-separated, single image)
        if (images) {
          try {
            let imageArray = [];
            ...
            if (imageArray.length > 0) {
              firstProductImage = imageArray[0];
            }
          } catch (e) {
            // If parsing fails, use as single image
            firstProductImage = images.split(',')[0].trim();
          }
        }
      }
    }
    ...
    const [insertCatalogResult] = await connection.execute(
      `INSERT INTO catalog (catalog_code, catalog_name, description, first_product_image, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, 'A', NOW(), NOW())`,
      [catalogCode, catalog_name || `Catalog ${nextId}`, `Catalog with ${product_ids.length} products`, firstProductImage]
    );
