...
/**
 * GET /product/:id
 * Serve product page with Open Graph meta tags for WhatsApp rich preview
 */
  ...
  try {
    ...
    if (products.length === 0) {
      return res.status(404).send(`
        ...
        <head>
          <meta charset="utf-8">
          <title>Product Not Found - BangkokMart</title>
    ...
    if (product.first_image) {
      imageUrl = `https://bangkokmart.in/uploads/products/${path.basename(product.first_image)}`;
    } else if (product.images) {
      try {
        const images = JSON.parse(product.images || '[]');
        if (images.length > 0) {
          imageUrl = `https://bangkokmart.in/uploads/products/${path.basename(images[0])}`;
        }
      } catch (e) {
...
<head>
    ...
    <meta property="og:description" content="${product.description || 'Check out this amazing product on BangkokMart'}">
    <meta property="og:image" content="${imageUrl}">
    <meta property="og:url" content="https://bangkokmart.in/product/${productId}">
    ...
    <meta name="twitter:description" content="${product.description || 'Check out this amazing product on BangkokMart'}">
    <meta name="twitter:image" content="${imageUrl}">
    
    <!-- Basic Meta Tags -->
    <title>${product.name} - BangkokMart</title>
    <meta name="description" content="${product.description || 'Check out this amazing product on BangkokMart'}">
...
<body>
    <div class="container">
        <img src="${imageUrl}" alt="${product.name}" class="product-image" onerror="this.src='https://bangkokmart.in/logo.png'">
        <h1 class="product-name">${product.name}</h1>
        <p class="product-description">${product.description || 'Check out this amazing product on BangkokMart'}</p>
