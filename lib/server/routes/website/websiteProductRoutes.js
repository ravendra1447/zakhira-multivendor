// server/routes/website/productRoutes.js
...
// Get all products for website (only active products and variants)
router.get('/website/products', async (req, res) => {
  try {
...
// Get single product details for website (only active variants)
router.get('/website/products/:productId', async (req, res) => {
  try {
    const { productId } = req.params;

    // Get product info (only if active)
    const [products] = await pool.execute(`
      SELECT 
        id,
        user_id,
    ...
    console.error('Error fetching website product details:', error);
    res.status(500).json({
      success: false,
...
router.get('/website/products/:productId/colors', async (req, res) => {
  try {
    const { productId } = req.params;
...
router.get('/website/products/:productId/colors/:colorName/sizes', async (req, res) => {
  try {
...
// Search products for website (filters inactive variants)
router.get('/website/search/products', async (req, res) => {
  try {
    ...
    console.error('Error searching website products:', error);
    res.status(500).json({
      success: false,
